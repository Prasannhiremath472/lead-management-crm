...
const read = async (req, res) => {
  // Find document by id
  const [rows] = await pool.query('SELECT * FROM orders WHERE id = ? AND removed = 0', [
    req.params.id,
  ]);
  ...
  const [clientRows] = await pool.query('SELECT * FROM clients WHERE id = ?', [order.client_id]);

  // order_items already carry the denormalized item_name/price/total
  // snapshot plus product_id, so just return them as-is for display.

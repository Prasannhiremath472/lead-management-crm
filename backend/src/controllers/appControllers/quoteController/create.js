...
const create = async (req, res) => {
  ...
  try {
    ...
    const fileId = 'quote-' + insertId + '.pdf';
    await conn.query('UPDATE quotes SET pdf = ? WHERE id = ?', [fileId, insertId]);
  ...
  const [quoteRows] = await pool.query('SELECT * FROM quotes WHERE id = ?', [insertId]);

...
const offerModel = require('@/db/models/offerModel');
...
const paginatedList = async (req, res) => {
  ...
  if (filter !== undefined && equal !== undefined) {
    ...
    if (!offerModel.filterableFields.includes(filter)) {
      return res.status(400).json({
        success: false,
        result: [],
  ...
  if (req.query.fields && req.query.q !== undefined) {
    ...
    const allowedFields = requestedFields.filter((f) => offerModel.searchableFields.includes(f));
  ...
  const sortBySafe = offerModel.sortableFields.includes(sortBy) ? sortBy : offerModel.defaultSort;
  const direction =
    String(sortValue) === '1' || String(sortValue).toLowerCase() === 'asc' ? 'ASC' : 'DESC';

  const [rows] = await pool.query(
    `SELECT * FROM offers WHERE ${whereSql} ORDER BY ${sortBySafe} ${direction} LIMIT ? OFFSET ?`,
    [...whereValues, limit, skip]
  );

  const [countRows] = await pool.query(
    `SELECT COUNT(*) AS count FROM offers WHERE ${whereSql}`,
    whereValues
  );

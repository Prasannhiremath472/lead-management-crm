...
const read = async (req, res) => {
  // Find document by id
  const [rows] = await pool.query('SELECT * FROM invoices WHERE id = ? AND removed = 0', [
    req.params.id,
  ]);
  ...
  const [clientRows] = await pool.query('SELECT * FROM clients WHERE id = ?', [invoice.client_id]);

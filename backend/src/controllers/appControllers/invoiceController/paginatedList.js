...
const paginatedList = async (req, res) => {
  ...
  if (filter !== undefined && equal !== undefined) {
    ...
    if (!invoiceModel.filterableFields.includes(filter)) {
      return res.status(400).json({
        success: false,
        result: [],
  ...
  if (req.query.fields && req.query.q !== undefined) {
    ...
    const allowedFields = requestedFields.filter((f) => invoiceModel.searchableFields.includes(f));
  ...
  const sortBySafe = invoiceModel.sortableFields.includes(sortBy) ? sortBy : invoiceModel.defaultSort;
  const direction =
    String(sortValue) === '1' || String(sortValue).toLowerCase() === 'asc' ? 'ASC' : 'DESC';

  const [rows] = await pool.query(
    `SELECT * FROM invoices WHERE ${whereSql} ORDER BY ${sortBySafe} ${direction} LIMIT ? OFFSET ?`,
    [...whereValues, limit, skip]
  );

  const [countRows] = await pool.query(
    `SELECT COUNT(*) AS count FROM invoices WHERE ${whereSql}`,
    whereValues
  );
